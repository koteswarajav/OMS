...
function OverridePopover({
  ...
  if (!open) return null
  return (
    <div className="absolute top-full left-0 mt-2 z-50 w-80 bg-white border border-gray-200 rounded-xl shadow-lg p-5 flex flex-col gap-4">
      ...
      <div className="px-3.5 py-3 bg-gray-50 rounded-lg border border-gray-150 flex items-center gap-2">
        <div className="flex-1">
          <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-0.5">Global default</div>
          <div className="text-sm font-semibold text-gray-800">{globalDefault}</div>
        </div>
      </div>
      {children}
      <div className="flex items-center justify-between pt-3 border-t border-gray-150">
        ...
        <button onClick={onApply}
          className="px-5 h-8 bg-blue-600 text-white text-xs font-semibold rounded-lg hover:bg-blue-700 transition-colors">
          Apply
        </button>
      </div>
    </div>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={`group flex items-center gap-2 px-3 py-2 rounded-lg border w-full text-left transition-all
          ${isCustom ? 'border-blue-200 bg-blue-50 hover:border-blue-300' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}
      >
        <div className="flex-1 min-w-0">
          <div className={`text-xs font-semibold truncate ${isCustom ? 'text-blue-700' : 'text-gray-700'}`}>{effective}</div>
          <div className={`text-[10px] mt-0.5 font-medium ${isCustom ? 'text-blue-500' : 'text-gray-400'}`}>
            {isCustom ? '✦ Custom' : '↑ Global'}
          </div>
        </div>
        <span className={`transition-opacity flex-shrink-0 ${isCustom ? 'text-blue-500 opacity-100' : 'text-gray-300 opacity-0 group-hover:opacity-100'}`}>
          <IcoPencil />
        </span>
      </button>
      <OverridePopover
...
function ChannelsTab() {
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Info banner */}
      <div className="flex items-start gap-3 px-4 py-3.5 rounded-lg bg-sky-50 border border-sky-200">
        <IcoInfo />
        <div className="flex-1">
          <p className="text-xs leading-relaxed text-sky-900">
            <span className="font-semibold">Global defaults:</span> Routing → <span className="font-semibold text-sky-950">{GLOBAL.routing.location}</span>, Aggregation → <span className="font-semibold text-sky-950">{GLOBAL.aggregation.location}</span>
          </p>
        </div>
      </div>
...
function ChannelRow({ ch }: { ch: Channel }) {
  const [open, setOpen] = useState(false)
  return (
    <div className={`bg-white border rounded-lg overflow-hidden transition-all ${open ? 'border-blue-200 shadow-md' : 'border-gray-200 hover:border-gray-300'}`}>
      <button onClick={() => setOpen(v => !v)}
        ...
        <div className="flex-1 min-w-0">
          <div className="text-base font-semibold text-gray-900">{ch.name}</div>
        ...
        <div className="flex items-center gap-8 flex-shrink-0 ml-4">
          <div className="text-right">
            <div className="text-xs font-semibold text-gray-700">{ch.aggregationOn}</div>
            <div className="text-[10px] text-gray-500 mt-0.5">Aggregation on</div>
          </div>
          <div className="text-right">
            <div className="text-xs font-semibold text-gray-700">{ch.routingOn}</div>
            <div className="text-[10px] text-gray-500 mt-0.5">Routing on</div>
          </div>
        </div>
      ...
      {open && (
        <div className="border-t border-gray-150 bg-gray-50 px-6 py-4 flex flex-col gap-4">
          <div className="grid grid-cols-3 gap-2 text-[10px] font-semibold text-gray-400 uppercase tracking-wider pb-2">
            <div>Connection</div>
...
function ConnectionRow({ conn }: { conn: Connection }) {
  ...
  return (
    <div className={`grid grid-cols-3 gap-3 items-center px-4 py-3 bg-white border rounded-lg transition-all
      ${hasOverride ? 'border-blue-200' : 'border-gray-200'}`}>

      {/* Connection info */}
      <div className="flex items-center gap-3 col-span-1 min-w-0">
        <div className="flex-1 min-w-0">
          <div className="text-xs font-semibold text-gray-900 truncate">{conn.name}</div>
          <div className="text-[10px] text-gray-500 mt-0.5">{conn.app}</div>
          {conn.isCustom && <Badge variant="primary" style={{ marginTop: '4px' }}>Custom</Badge>}
        </div>
      </div>
    ...
    <div className="flex flex-col gap-12">
      ...
      <div>
        <SectionHeader
          ...
          subtitle="Configure how orders are collected from multiple channels and batched together."
        />
        ...
        <div className="space-y-6">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Aggregated Order ID Format</h3>
            <Card className="p-6">
              <div className="grid grid-cols-2 gap-6">
                <Field label="ID Prefix" hint="Optional string before the numeric part (e.g. BATCH)">
              ...
              <div className="mt-6 pt-6 border-t border-gray-150">
                <div className="px-4 py-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="text-xs text-blue-900">
                    <div className="font-mono font-bold text-blue-700 mb-2">BATCH-000001</div>
          ...
          {/* Aggregation Window */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Aggregation Window</h3>
            <Card className="p-6">
              <div className="space-y-4">
                <div className="flex items-start justify-between pb-4 border-b border-gray-150 last:border-b-0 last:pb-0">
                  <div className="flex-1 pr-4">
                    <div className="text-sm font-semibold text-gray-900">Auto-close after</div>
                    <div className="text-xs text-gray-500 mt-1">Batch automatically closes after this duration from first order</div>
                  </div>
                  <div className="text-sm font-semibold text-gray-700 flex-shrink-0">30 minutes</div>
                </div>
                <div className="flex items-start justify-between pt-4">
                  <div className="flex-1 pr-4">
                    <div className="text-sm font-semibold text-gray-900">Maximum orders per batch</div>
                    <div className="text-xs text-gray-500 mt-1">Batch closes when this limit is reached, even if time window not elapsed</div>
                  </div>
                  <div className="text-sm font-semibold text-gray-700 flex-shrink-0">200 orders</div>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Default Warehouse Location</h3>
            <Card className="p-6">
              <Field label="Warehouse" hint="Fallback location when no aggregation rule matches">
              ...
              <div className="mt-4 pt-4 border-t border-gray-150">
                <p className="text-xs text-gray-500 leading-relaxed">
                  <span className="font-semibold text-gray-700">When no specific aggregation rule matches an order:</span> It automatically uses this warehouse location. If not set, orders will be held for manual review.
                </p>
              </div>
            </Card>
          </div>
      ...
      <div>
        <SectionHeader
          title="Order Routing"
          subtitle="Configure how orders are routed to different fulfillment locations based on conditions."
        />
        ...
        <div className="space-y-6">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Default Warehouse Location</h3>
            <Card className="p-6">
              <Field label="Warehouse" hint="Fallback location when no routing rule matches">
              ...
              <div className="mt-4 pt-4 border-t border-gray-150">
                <p className="text-xs text-gray-500 leading-relaxed">
                  <span className="font-semibold text-gray-700">When no routing rule matches an order:</span> It automatically uses this warehouse location. If not set, orders will be held for manual review.
                </p>
              </div>
            </Card>
          </div>
          ...
          {/* Split Configuration */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Split Configuration</h3>
            <Card style={{ padding: 0 }}>
              <div className="divide-y divide-gray-150">
                ...
                <div className={`flex items-start justify-between gap-6 px-6 py-5 transition-opacity ${!allowMulti ? 'opacity-50' : ''}`}>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-semibold text-gray-900">Allow multiple locations per line item</span>
                      {!allowMulti && (
                    ...
                    {!allowMulti && <p className="text-xs text-amber-600 mt-2 font-medium">Requires order splitting to be enabled first.</p>}
                  </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1.5">Rules are evaluated in priority order. First match wins.</p>
              </div>
            ...
            <Card style={{ padding: 0 }}>
              {/* Table header */}
              <div className="flex items-center gap-4 px-6 py-3.5 bg-gray-50 border-b border-gray-150 text-[10px] font-semibold text-gray-400 uppercase tracking-wider rounded-t-lg">
                <div className="w-4" />
              ...
              {rules.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-1">All orders will use the default fulfillment location.</p>
                </div>
              ) : (
                <div className="divide-y divide-gray-150">
                  {rules.map(rule => (
                    <div key={rule.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-gray-50/60 transition-colors group">
                      <div className="w-4 flex-shrink-0 cursor-grab opacity-0 group-hover:opacity-100 transition-opacity">
                        <IcoGrip />
                      </div>
                      <div className="w-8 text-center">
                        <span className="text-xs font-bold text-gray-400 bg-gray-100 rounded-full w-6 h-6 inline-flex items-center justify-center">
                          {rule.priority}
                        </span>
                      </div>
                      ...
                      <div className="flex-1 flex items-center gap-2 min-w-0">
                        <select
                          ...
                          className="border border-gray-200 rounded-lg px-3 py-2 text-xs font-medium text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                        >
                        ...
                        <input
                          ...
                          placeholder="value…"
                          className="border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-0 flex-1"
                        />
                      </div>
                      ...
                      <div className="w-32">
                        <select
                          ...
                          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs font-semibold text-blue-700 bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                        >
                          <option value="WMS 1">WMS 1</option>
                          <option value="WMS 2">WMS 2</option>
                        </select>
                      ...
                      <button
                        ...
                        className="w-8 flex items-center justify-center text-gray-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"
                      >
                        <IcoTrash />
                      </button>
                    </div>
              ...
              {/* Footer */}
              {rules.length > 0 && (
                <div className="px-6 py-3.5 bg-gray-50 border-t border-gray-150 rounded-b-lg">
                  <p className="text-xs text-gray-500">
                    <strong className="text-gray-700">{rules.length} rule{rules.length !== 1 ? 's' : ''}</strong> active · Drag to reorder · Unmatched orders use default location
                  </p>
                </div>
              )}
            </Card>
      ...
      <div className="max-w-7xl mx-auto px-8 py-10">
        ...
        <div className="mb-12">
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wider mb-3">
            <span className="hover:text-gray-700 cursor-pointer transition-colors">Settings</span>
            <span className="mx-2 text-gray-300">/</span>
            <span className="text-gray-700">Channel Management</span>
          </p>
          <div className="flex items-end justify-between gap-6">
            <div className="flex-1">
              <h1 className="text-3xl font-bold tracking-tight text-gray-950 mb-2">OMS Configuration</h1>
              <p className="text-base text-gray-600 leading-relaxed max-w-2xl">
                Manage channel integrations, order aggregation settings, and intelligent routing rules for efficient fulfillment.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Tab bar */}
        <div className="border-b border-gray-200 mb-10 -mx-1 px-1">
        ...
        {/* Footer actions */}
        {tab !== 'channels' && (
          <div className="flex items-center justify-end gap-3 mt-16 pt-8 border-t border-gray-200">
